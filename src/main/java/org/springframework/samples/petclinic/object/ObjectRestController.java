...
public class ObjectRestController {
	...
	// @PostMapping
	// public ResponseEntity<Object> createObject(@RequestBody @Valid Object
	// newObject, BindingResult br){
	// Object result=null;
	// if(!br.hasErrors())
	...
	// @PutMapping("/{id}")
	// public ResponseEntity<Void> modifyObject(@RequestBody @Valid Object
	// newObject, BindingResult br,@PathVariable("id") int id) {
	// Object objectToUpdate=this.findObject(id).getBody();
	// if(br.hasErrors())
	// throw new BadRequestException(br.getAllErrors());
	// else if(newObject.getId()==null || !newObject.getId().equals(id))
	// throw new BadRequestException("Object id is not consistent with resource
	// URL:"+id);
	// else{
